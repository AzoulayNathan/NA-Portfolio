  ...
  return (
    <div className={`relative overflow-hidden ${local ? 'bg-sand' : ''} ${className}`}>
      <img
        ...
        className={
          local
            ? `absolute inset-0 w-full h-full object-contain p-4 md:p-6 ${imgClassName}`
  ...
  return (
    <>
      <img
        ...
        className={
          local
            ? 'absolute inset-0 w-full h-full object-contain p-3 bg-sand opacity-100 transition-transform duration-700 group-hover:scale-[1.06]'
            : 'absolute inset-0 w-full h-full object-cover opacity-90 transition-transform duration-700 group-hover:scale-[1.24]'
        }
      />
      {local ? (
        <div className="absolute inset-x-0 bottom-0 h-1/3 bg-gradient-to-t from-tropical/50 to-transparent pointer-events-none" />
      ) : (
...
export function TeachingToolImage({ src, alt, large }) {
  const local = isLocalPortfolioAsset(src);
  const h = large ? 'h-36' : 'h-20';
  return (
    <div className={`${h} w-full mb-3 overflow-hidden rounded-sm ${local ? 'bg-sand' : ''}`}>
      <img
        src={src}
        alt={alt}
        className={`w-full h-full ${local ? 'object-contain p-1.5' : 'object-cover'}`}
        loading="lazy"
      />
    </div>
  );
