  ...
  const rooms = [
    {
      ...
      external: true,
      title: t('focused_rooms_websites'),
      status: t('rooms_status_active'),
      statusClass: 'text-olive',
    },
    {
      to: '/classroom',
      title: t('focused_rooms_classroom'),
      status: t('rooms_status_active'),
      statusClass: 'text-olive',
    },
  ];
    ...
    <section ref={ref} className="py-20 px-6 md:px-10 bg-sand/40 border-t border-olive/10">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid sm:grid-cols-2 gap-4 max-w-2xl">
          {rooms.map((room, i) => {
            const cardClass =
              'block bg-quartz border border-olive/12 px-5 py-4 hover:border-tropical/30 transition-colors group';
            const content = (
              <>
                <span className="font-serif text-lg text-ink group-hover:text-tropical transition-colors">
                  {room.title}
                </span>
                <span className={`font-sans text-[10px] tracking-widest uppercase block mt-2 ${room.statusClass}`}>
                  {room.status}
                </span>
              </>
            );
            ...
            return (
              <motion.div
                key={room.external ? room.href : room.to}
              ...
              >
                {room.external ? (
                  ...
                  <Link to={room.to} className={cardClass}>
                    {content}
                  </Link>
                )}
              </motion.div>
