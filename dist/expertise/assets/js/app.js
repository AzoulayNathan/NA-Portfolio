(function(){
      ...
      <main class="expertise-hub">
        ...
        <section class="hub-intro">
          ...
          <div class="hub-sculpture" aria-hidden="true">
            <img src="assets/img/expertise-hero.png" alt=""/>
          </div>
        </section>
        <section class="hub-fields" aria-label="${esc(t('hub_fields_aria'))}">
          ${fields.map((f,i)=>`<div class="hub-field ${i===3?'is-highlighted':''}" style="--field:${f.accent};--field-rgb:${f.rgb}" data-route="${f.id}" data-hub-field="${f.id}">
            ...
            <span class="hub-field-actions">
              <span class="hub-field-action">${esc(t('hub_explore'))} ${icon('arrow')}</span>
              <a class="hub-open-course" href="${esc(courseHref(f.id))}" target="_top" data-course-link>${esc(t('open_course'))}</a>
            </span>
          ...
          <section class="profile-hero">
            <div class="profile-hero-copy">
              ...
              <div class="profile-actions">
                ...
                <a class="hub-open-course profile-open-course" href="${esc(courseHref(field.id))}" target="_top" data-course-link>${esc(t('open_course'))} ${icon('arrow')}</a>
                ${cvControls(field)}
              </div>
            </div>
            ${VIS[field.visual]()}
  ...
  function contactPage(params){
    ...
    const booking=CONFIG.bookingUrl||'https://calendly.com/nathan-azoulay-na-studio1/30min';
  ...
  function bindAll(){
    ...
    document.querySelectorAll('[data-route]').forEach(el=>{
      el.addEventListener('click',e=>{
        if(e.target.closest('[data-course-link]')) return;
